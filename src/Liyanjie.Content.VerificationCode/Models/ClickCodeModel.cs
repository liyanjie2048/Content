...
namespace Liyanjie.Content.Models
{
    ...
    public class ClickCodeModel
    {
        ...
        /// <returns></returns>
        public async Task<(Point[] FontPoints, Image FontImage, Image BoardImage)> GenerateAsync(VerificationCodeOptions options)
        {
            ...
            graphicsFont.Clear(VerificationCodeHelper.RandomColor(false));
            //绘制干扰线
            for (int i = 0; i < 4; i++)
            {
                ...
                using var pen = new Pen(VerificationCodeHelper.RandomColor(true));
            ...
            for (int i = 0; i < strs.Length; i++)
            {
                var item = strs[i];
                var random = new Random(Guid.NewGuid().GetHashCode());
                using var pen = new Pen(VerificationCodeHelper.RandomColor());
