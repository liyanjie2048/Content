...
public class UploadMiddleware : IMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context, RequestDelegate next)
    {
        ...
        var dir = "temps";
        if (request.Query.TryGetValue("dir", out var _dir) && !string.IsNullOrEmpty(_dir.FirstOrDefault()))
            dir = _dir.FirstOrDefault();

        var files = new List<UploadModel.UploadFileModel>(request.Form.Files.Count);
